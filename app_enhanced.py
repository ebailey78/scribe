...
class ScribeGUI:
    ...
    def start_recording(self):
        ...
        try:
            ...
            if not self.recorder.mic:
                self.update_status("❌ No loopback device found!", dot="🔴")
                return
            ...
            self.update_status(f"Error: {str(e)}", dot="🔴")
    ...
    def run_synthesis(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self.update_status(f"Synthesis Error: {str(e)}", dot="🔴"))
            self.root.after(0, self.reset_ui)
    
    def synthesis_complete(self):
        """Synthesis completed successfully."""
        self.update_status("Complete!", dot="🟢")
        self.progress_label.configure(text="Synthesis complete!")
        
        # Reset tray icon
        if self.tray_icon:
            self.tray_icon.icon = self.create_icon("gray")
        
        # Hide progress, show open button
